...
# --- 1. 基本設定 ---
st.set_page_config(page_title="配置馬券術 AI分析システム", layout="wide")
...
def load_data(file):
    try:
        ...
        best_row = 0
        max_hits = 0
        keywords = ['場所', '場名', 'R', 'レース', '馬番', '番', '馬名', 'オッズ', '単勝', '騎手', '厩舎']
        ...
        for name, group in df.groupby(g_keys):
            ...
            if common:
                for _, row in group.iterrows():
                    ...
                    if idx is not None:
                        df.at[idx, '青塗フラグ'] = 1
                        df.at[idx, 'タイプ_list'].append(f'★{col}青塗'); df.at[idx, 'スコア'] += 9.2
            ...
            for i in range(len(rows)-1):
                ...
                if pats:
                    ...
                    for r_data in [r1, r2]:
                        ...
                        if idx is not None:
                            df.at[idx, 'タイプ_list'].append('◎ペア'); df.at[idx, 'スコア'] += 4.0 if is_c else 3.0

    # AI予測 (全頭一括)
    if model:
        try:
            X = df[['単ｵｯｽﾞ', '青塗フラグ', 'ペアフラグ', '前日配置フラグ']].fillna({'単ｵｯｽﾞ':99, '青塗フラグ':0, 'ペアフラグ':0, '前日配置フラグ':0})
...
if up_curr:
    ...
    if status == "success":
        ...
        with st.form("result_form"):
                ...
                with p_tab:
                    ...
                    for r_tab, r_num in zip(r_tabs, sorted(p_df['R'].unique())):
                        with r_tab:
                            ...
                            # 表示列の定義
                            disp_cols = ['評価','正番','馬名','単ｵｯｽﾞ','AI激走確率','タイプ','総合スコア','着順']
            ...
            if st.form_submit_button("🔄 判定を更新"):
    ...
    st.info("サイドバーから今日のファイルを読み込んでください。")
